...
<head>
    ...
    {{-- css file --}}
    <link rel="stylesheet" type="text/css" href="{{ asset('dist/css/partner_registration.css') }}">
...
<body>
    ...
    <div class="partner_form">
        <section class="container partner_form_container text-light p-2 dummy">
                    ...
                    <div class="icon-object text-center" style="border:none !important;">
                        <div class="login-logo">
                            <a href="http://127.0.0.1:8000/partner_registration">
                                <img src="https://crm.qntmnet.com/QN-CDN/images/logo.png?v=08.18.0.234" class="qntmlogo">
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <form class="row g-3 p-4 " method="post" action="{{ route('registerPartner') }}" id="submitregistration">
                @csrf
                ...
                <div class="col-md-6">
                    {{-- select country --}}
                    <select class="form-select form-control" name="select_country" id="select_country" aria-label="Default select example">
                        <option selected disabled>Select Country</option>
                        @foreach ($countries as $country)
                        <option value="{{ $country->id }}" {{ old('select_country') == $country->id ? 'selected' : '' }} data-country-phonecode="{{ $country->phonecode }}">
                            {{ $country->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    {{-- Select State --}}
                    <select class="form-select form-control" name="select_state" id="select_state" aria-label="Default select example">
                ...
                <div class="col-md-6">
                    {{-- Select City --}}
                    <select class="form-select form-control" name="select_city" id="select_city" aria-label="Default select example">
                ...
                <div class="col-md-6 error_div">
                    <div class="mobile_no form-control">
                        ...
                        <input style="border-color: transparent; outline: none;" maxlength="10" name="mobile" type="tel" class="mobile" value="{{ old('mobile') }}" placeholder="Mobile">
                    </div>
                ...
                <div class="col-md-6">
                    <input type="text" maxlength="15" value="{{ old('personal_landline') }}" class="form-control" name="personal_landline" placeholder="Landline">
                    <span class="text-danger">{{ $errors->first('personal_landline') }}</span>
                </div>
                <div class="col-md-6">
                    <input type="email" id="Email" class="form-control" value="{{ old('mail') }}" name="mail" placeholder="Email">
                ...
                <div class="partner_form_footer">
                    ...
                    <div class="col-6 submit_btn">
                        <button type="submit" id="submitregistration" class="btn">Submit</button>
                    </div>
                </div>
            </form>
        </section>
    ...
    <script>
        ...
        var checkEmailvalid = '{{ route("checkUserExisting") }}';
    </script>

    <script type="text/javascript">
        function validateForm() {
            return jsValidator.form('submitregistration').isValid();
        }

        $(document).ready(function() {


            $('#submitregistration').on('submit', function(event) {
                event.preventDefault();

                var form = $(this);
                $.ajax({
                    url: "{{ route('checkUserExisting') }}",
                    type: "POST",
                    data: form.serialize(),
                    headers: {
                    ...
                    success: function(response) {
                        if (response.exists) {
                            ...
                            // User does not exist, submit the form data to register partner
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.status === 'success') {
                                        window.location.href = '/partner_registration' + '?flash_message=' + encodeURIComponent(response.message) + '&flash_class=' + encodeURIComponent(response.class);;
                                    } else {
                                        alert('error');
                                    }
                                },
                                error: function(xhr, status, error) {
                                    ...
                                    alert('Please Enter Valide Captcha.');
                                }
                            });
                        }
                    },
    ...
    <script src="{{ asset('dist/js/pages/partner_registration.js') }}"></script>
